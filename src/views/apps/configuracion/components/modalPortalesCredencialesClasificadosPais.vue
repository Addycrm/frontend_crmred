<template>
  <b-modal
    :id="infoModal.id"
    size="lg"
    ref="modal-portales-credentials-clasificados-pais"
    title="Credenciales Clasificado el País prueba"
    ok-title="Registrar"
    ...
    <div class="demo-inline-spacing">
      <b-row>
        ...
        <!-- <b-col md="6" xl="12" class="mb-1">
          <b-form-group
            label="tokek(Authorization)"
            label-for="token_fincaraiz"
          >
            <b-form-textarea
              v-model="data.authorization"
              id="textarea-rows"
...
export default {
  ...
  data() {
    return {
      infoModal: {
        id: "modal-portales-credentials-clasificados-pais",
        title: "",
        content: {},
        contentPreguntas: {},
      },
  ...
  methods: {
    ...
    hideModal() {
      this.$refs["modal-portales-credentials-clasificados-pais"].hide();
    },

    async getCredentialPortal() {
      await this.$store
        .dispatch("appPortales/getCredencialesClasificadosPais", {
    ...
    async addCredencialesPortalesUser(data) {
      await this.$store
        .dispatch("appPortales/addCredencialesClasificadosPais", {
          portal: this.infoModal.content.id,
