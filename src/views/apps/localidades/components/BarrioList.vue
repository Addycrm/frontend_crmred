...
import modalAddBarrioVue from "./modals/modalAddBarrio.vue";
...
export default {
  ...
  components: {
    ...
    FormCodePortales,
    VueGoodTable,
    "modal-barrio": modalAddBarrioVue,
    buttonRegister,
  },
      ...
      return [
        {
          titulo: "Barrios",
          config: {
            prop: "portales_barrio",
            config: {
              url: "api/auth/portales/barrios_portales",
              method: "post",
            },
          },
          tabla: {
  ...
  methods: {
    async deleteBarrio(id) {
      await this.$store
        .dispatch("appLocalidades/deleteBarrio", id.id)
        .then(() => {
          this.$store.commit("appLocalidades/DELETE_BARRIOS", id.id);
