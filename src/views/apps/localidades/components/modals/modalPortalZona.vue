...
export default {
  ...
  methods: {
    ...
    getPortales() {
      this.$store
        ...
        .then((response) => {
          ...
          let count = 0;
          var data = this.infoModal.content.portales_zona;
          for (let i = 0; i < response.length; i++) {
            for (let j = 0; j < data.length; i++) {
              if (response[i].id == data[i].portale_id) {
                response[i].codigo = data[i].codigo;
              }
            }
          }
          this.portales = response;
    ...
    hideModal() {
      this.$refs["modal-cities-portales"].hide();
    },
    ...
    handleSubmit() {
      this.$store
        .dispatch("appPortales/addZonaPortales", {
          portales: this.portales,
          id: this.infoModal.content.id,
        })
        .then(() => {
          this.$store
            .dispatch("appLocalidades/getZona", {
